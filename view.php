 ...
 ********************************************************************************/
	...
	require('hooks/custom_hooks.php');					function curlPost($url, $data,$showError=1){		$ch = curl_init();		$header = [];			curl_setopt($ch, CURLOPT_URL, $url);		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, FALSE);		curl_setopt($ch, CURLOPT_HTTPHEADER, $header);		curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (compatible; MSIE 5.01; Windows NT 5.0)');		curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);		curl_setopt($ch, CURLOPT_AUTOREFERER, 1);		curl_setopt($ch, CURLOPT_POSTFIELDS, $data);		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);		$tmpInfo = curl_exec($ch);				$errorno=curl_errno($ch);		return $tmpInfo;	}	function unite_pay($amount,$xmpch,$key,$orderNo){						$url='http://cwcwx.hhu.edu.cn/zhifu/payAccept.aspx';				$data=array();		$data['orderDate']=date('YmdHis',time()+3600*8);		$data['orderNo']=$orderNo;		$data['amount']=$amount;		$data['notify_url']='http://mf.dastle.net/unitepay_notify.php';		$data['return_url']='http://mf.dastle.net/return_url.php';		$data['xmpch']=$xmpch;				$data['sign']=md5('orderDate='.$data['orderDate'].'&orderNo='.$data['orderNo'].'&amount='.$data['amount'].'&xmpch='.$data['xmpch'].'&return_url='.$data['return_url'].'&notify_url='.$data['notify_url'].$key);		$r=curlPost($url,$data);		$r=str_replace('http','https',$r);		$r=str_replace('<head>','<head><base href="http://cwcwx.hhu.edu.cn/zhifu/" />',$r);				return $r;	}				
	...
	if(mf_is_form_submitted()){ 
		...
		if(!isset($input_array['password'])){ 
			...
			if($submit_result['status'] === true){																			$form_properties = mf_get_form_properties($dbh,$input_array['form_id'],array('payment_merchant_type'));															if($form_properties['payment_merchant_type']=='Unite_pay'){																										$form_propertiess = mf_get_form_properties($dbh,$input_array['form_id'],array('payment_merchant_type','payment_price_type','payment_price_amount','payment_paypal_rest_live_clientid','payment_paypal_rest_live_secret_key'));												if($form_propertiess['payment_price_type']=='fixed'){							$money=$form_propertiess['payment_price_amount'];													}else{													$query = "select 										A.element_id,										A.option_id,										A.`price`,										B.`position` 									from 										".MF_TABLE_PREFIX."element_prices A left join ".MF_TABLE_PREFIX."element_options B 									  on 										(A.form_id=B.form_id and A.element_id=B.element_id and A.option_id=B.option_id) 								   where 										A.form_id = ? 								order by 												A.element_id,B.position asc";								$params = array($input_array['form_id']);																$sth = mf_do_query($query,$params,$dbh);								$current_price_settings = array();																while($row = mf_do_fetch_result($sth)){									$element_id = (int) $row['element_id'];									$option_id = (int) $row['option_id'];									$current_price_settings[$element_id][$option_id]  = $row['price'];																		$ky='element_'.$element_id;									if(isset($input_array[$ky])){										if($option_id==$input_array[$ky]){											$money=$row['price'];																					}																			}																										}														}							$payment_data['payment_fullname'] = 'Unite_pay';		$payment_data['form_id'] 		  = $input_array['form_id'];		$payment_data['record_id'] 		  = $submit_result['entry_id'];		$payment_data['date_created']	  = date("Y-m-d H:i:s");		$payment_data['status']			  = 0;		$payment_data['payment_status']   = 'unpaid'; 		$query = "INSERT INTO `".MF_TABLE_PREFIX."form_payments`(								`form_id`, 								`record_id`, 								`payment_id`, 								`date_created`, 								`payment_date`, 								`payment_status`, 								`payment_fullname`, 								`payment_amount`, 								`payment_currency`, 								`payment_test_mode`,								`payment_merchant_type`, 								`status`, 								`billing_street`, 								`billing_city`, 								`billing_state`, 								`billing_zipcode`, 								`billing_country`, 								`same_shipping_address`, 								`shipping_street`, 								`shipping_city`, 								`shipping_state`, 								`shipping_zipcode`, 								`shipping_country`) 						VALUES (								:form_id, 								:record_id, 								:payment_id, 								:date_created, 								:payment_date, 								:payment_status, 								:payment_fullname, 								:payment_amount, 								:payment_currency, 								:payment_test_mode,								:payment_merchant_type, 								:status, 								:billing_street, 								:billing_city, 								:billing_state, 								:billing_zipcode, 								:billing_country, 								:same_shipping_address, 								:shipping_street, 								:shipping_city, 								:shipping_state, 								:shipping_zipcode, 								:shipping_country)";						$params = array();		$params[':form_id'] 		  	= $payment_data['form_id'];		$params[':record_id'] 			= $payment_data['record_id'];		$params[':payment_id'] 			= 'Unite_pay';		$params[':date_created'] 		= $payment_data['date_created'];		$params[':payment_date'] 		= date("Y-m-d H:i:s");		$params[':payment_status'] 		= $payment_data['payment_status'];		$params[':payment_fullname']  	= $payment_data['payment_fullname'];		$params[':payment_amount'] 	  	= $money;		$params[':payment_currency']  	= 'CNY';		$params[':payment_test_mode'] 	= '';		$params[':payment_merchant_type'] = 'unite_pay';		$params[':status'] 			  	= $payment_data['status'];		$params[':billing_street'] 		= '';		$params[':billing_city']		= '';		$params[':billing_state'] 		= '';		$params[':billing_zipcode'] 	= '';		$params[':billing_country'] 	= '';		$params[':same_shipping_address'] = '';		$params[':shipping_street'] 	= '';		$params[':shipping_city'] 		= '';		$params[':shipping_state'] 		= '';		$params[':shipping_zipcode'] 	= '';		$params[':shipping_country'] 	= '';		mf_do_query($query,$params,$dbh);																															$r=unite_pay($money,$form_propertiess['payment_paypal_rest_live_clientid'],$form_propertiess['payment_paypal_rest_live_secret_key'],$payment_data['record_id'].'_'.$input_array['form_id']);					echo $r;die;						}		
