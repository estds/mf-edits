...
function mf_display_text($element)
{
    ...
    if ($element->text_default_type == 'random')
    {
        ...
        if ($element->text_default_random_type == 'letter')
        {
            ...
            {
                case 'u':
                    $rs_alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
                break;
                case 'l':
                    $rs_alphabet = 'abcdefghijklmnopqrstuvwxyz';
                break;
                case 'b':
                    $rs_alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
                break;
            }
        }
        ...
        else if ($element->text_default_random_type == 'alphanum')
        {
            ...
            {
                case 'u':
                    $rs_alphabet = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
                break;
                case 'l':
                    $rs_alphabet = '0123456789abcdefghijklmnopqrstuvwxyz';
                break;
                case 'b':
                    $rs_alphabet = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
                break;
            }
        }
        else if ($element->text_default_random_type == 'all')
        {
            ...
            {
                case 'u':
                    $rs_alphabet = '"\'`~!@#$%^&*()_-+={}[]|\\:;<>,.?/0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
                break;
                case 'l':
                    $rs_alphabet = '"\'`~!@#$%^&*()_-+={}[]|\\:;<>,.?/0123456789abcdefghijklmnopqrstuvwxyz';
                break;
                case 'b':
                    $rs_alphabet = '"\'`~!@#$%^&*()_-+={}[]|\\:;<>,.?/0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
                break;
            }
        }
    ...
    else if ($element->text_default_type == 'static')
    {
        ...
        {
            $element->default_value = str_replace(array(
                '{http_referer}',
                '{request_uri}'
            ) , array(
    ...
    if (isset($_GET['element_' . $element
        ->id]) && $element->text_default_type == 'static')
    {
    ...
    //check for placeholder
    if ((!empty($element->enable_placeholder) && ($element->default_value !== '') && ($element->default_value == $original_default_value)) || (!empty($element->enable_placeholder) && ($element->default_value === '') && ($original_default_value !== '')))
    {
    ...
    if (!empty($element->range_min) && !empty($element->range_max))
    {
        ...
        $range_limit_markup = "<span class=\"label\">{$range_min_max_tag}&nbsp;&nbsp; <em class=\"currently_entered\">{$currently_entered_tag}</em></span>";
    ...
    elseif (!empty($element->range_max))
    {
        ...
        $range_limit_markup = "<span class=\"label\">{$range_max_tag}&nbsp;&nbsp; <em class=\"currently_entered\">{$currently_entered_tag}</em></span>";
        $input_handler = "onkeyup=\"limit_input({$element->id},'{$element->range_limit_by}',{$element->range_max});\" onchange=\"limit_input({$element->id},'{$element->range_limit_by}',{$element->range_max});\"";
        if ($element->range_limit_by == 'c')
        {
            $maxlength = 'maxlength="' . $element->range_max . '"';
    ...
    elseif (!empty($element->range_min))
    {
        ...
        $range_limit_markup = "<span class=\"label\">{$range_min_tag}&nbsp;&nbsp; <em class=\"currently_entered\">{$currently_entered_tag}</em></span>";
    ...
    $element_markup = <<<EOT
		...
		<div>
			<input id="element_{$element->id}" name="element_{$element->id}" {$attr_placeholder} {$attr_readonly} {$maxlength} class="element text {$element->size}" type="{$element_type}" value="{$element->default_value}"  {$input_handler} />
			{$range_limit_markup} 
...
function mf_display_textarea($element)
{
    ...
    $element_markup = <<<EOT
		...
		<label class="description" for="element_{$element->id}">{$element->title} {$span_required}</label>
		<div>
			<textarea id="element_{$element->id}" name="element_{$element->id}" {$attr_placeholder} {$attr_readonly} class="element textarea {$element->size}" rows="8" cols="90" {$input_handler}>{$element->default_value}</textarea>
			{$range_limit_markup} 
		</div>{$guidelines} {$error_message}
		</li>
EOT;
    ...
    else
    {
        ...
        $signature_markup = <<<EOT
			<div class="mf_signature_switch" style="text-align: right">
				<a class="sig_switch_draw {$signature_draw_active}" href="javascript: switch_signature_type({$element->id},'draw');">Draw</a> or 
				<a class="sig_switch_type {$signature_type_active}" href="javascript: switch_signature_type({$element->id},'type');">Type</a>
			</div>
	        <div class="mf_signature_draw" style="display: {$signature_draw_display}">
		        ...
		        {$guidelines}
		        <a class="mf_signature_clear element_{$element->id}_clear" href="javascript:clear_signature({$element->id})">Clear</a>
	        </div>
...
function mf_display_url($element)
{
    ...
    $element_markup = <<<EOT
		...
		<label class="description" for="element_{$element->id}">{$element->title} {$span_required}</label>
		<div>
			<input id="element_{$element->id}" name="element_{$element->id}" {$attr_placeholder} {$attr_readonly} class="element text {$element->size}" type="text"  value="{$element->default_value}" /> 
		</div>{$guidelines} {$error_message}
		</li>
EOT;
    return $element_markup;
...
EOT;
    ...
    $element_markup = <<<EOT
		...
		<label class="description" for="element_{$element->id}">{$element->title} {$span_required}</label>
		<div>
			<input id="element_{$element->id}" name="element_{$element->id}" {$attr_placeholder} {$attr_readonly} class="element text {$element->size}" type="text" maxlength="255" value="{$element->default_value}" /> 
...
function mf_display_phone($element)
{
    ...
    if (!empty($element->populated_value['element_' . $element->id . '_1']['default_value']) || !empty($element->populated_value['element_' . $element->id . '_2']['default_value']) || !empty($element->populated_value['element_' . $element->id . '_3']['default_value']))
    {
        $default_value_1 = '';
        $default_value_2 = '';
        $default_value_3 = '';
    ...
    $element_markup = <<<EOT
		...
		<fieldset>
			...
			<span class="phone_1">
				<input id="element_{$element->id}_1" name="element_{$element->id}_1" {$attr_readonly} class="element text" size="3" maxlength="3" value="{$default_value_1}" type="text" /> -
				<label for="element_{$element->id}_1">###</label>
			</span>
			<span class="phone_2">
				<input id="element_{$element->id}_2" name="element_{$element->id}_2" {$attr_readonly} class="element text" size="3" maxlength="3" value="{$default_value_2}" type="text" /> -
				<label for="element_{$element->id}_2">###</label>
			</span>
			<span class="phone_3">
		 		<input id="element_{$element->id}_3" name="element_{$element->id}_3" {$attr_readonly} class="element text" size="4" maxlength="4" value="{$default_value_3}" type="text" />
...
function mf_display_date($element)
{
    ...
    if (!empty($element->default_value))
    {
        ...
        { //it's not a valid date, display blank
            $default_value_1 = '';
            $default_value_2 = '';
            $default_value_3 = '';
        }
    ...
    {
        if (!empty($element->date_range_min))
        { //value: yyyy-mm-dd
            ...
            if ($date_range_min[1] == '00' && ($date_range_min[2] == '00' || $date_range_min[2] == '01'))
            {
                ...
                else if ($date_range_min[2] == '01')
                { //if the value is negatove
                    $cal_min_date = ", minDate: +{$date_range_min[0]}";
                }
            }
            else
        ...
        if (!empty($element->date_range_max))
        { //value: yyyy-mm-dd
    ...
    if ((!empty($element->date_disable_specific) && !empty($element->date_disabled_list)) || (!empty($element->date_disable_dayofweek) && isset($element->date_disabled_dayofweek_list)))
    {
...
function mf_display_radio($element)
{
    ...
    foreach ($element->options as $option)
    {
        ...
        //check for GET parameter to populate default value
        if (isset($_GET['element_' . $element->id]) && $_GET['element_' . $element
            ->id] == $option->id)
        {
            $checked = 'checked="checked"';
            $selected_price_value = $option->price_definition;
...
function mf_display_select($element)
{
    ...
    foreach ($element->options as $option)
    {
        ...
        if ($option->is_default || (isset($_GET['element_' . $element->id]) && $_GET['element_' . $element
            ->id] == $option->id))
        {
...
function mf_display_simple_name($element)
{
    ...
    //check for populated values, if exist override the default value
    if (!empty($element->populated_value['element_' . $element->id . '_1']['default_value']) || !empty($element->populated_value['element_' . $element->id . '_2']['default_value']))
    {
        $default_value_1 = '';
        $default_value_2 = '';
        $default_value_1 = $element->populated_value['element_' . $element->id . '_1']['default_value'];
...
function mf_display_name($element)
{
    ...
    //check for populated values, if exist override the default value
    if (!empty($element->populated_value['element_' . $element->id . '_1']['default_value']) || !empty($element->populated_value['element_' . $element->id . '_2']['default_value']) || !empty($element->populated_value['element_' . $element->id . '_3']['default_value']) || !empty($element->populated_value['element_' . $element->id . '_4']['default_value']))
    {
        $default_value_1 = '';
        $default_value_2 = '';
        $default_value_3 = '';
...
function mf_display_name_wmiddle($element)
{
    ...
    $element_markup = <<<EOT
		...
		<span class="namewm_first">
			<input id="element_{$element->id}_2" name="element_{$element->id}_2" {$attr_readonly} type="text" class="element text large" maxlength="255" value="{$default_value_2}" />
			<label for="element_{$element->id}_2">{$mf_lang['name_first']}</label>
		</span>
...
function mf_display_time($element)
{
    ...
    if (!empty($element->default_value))
    {
        ...
        { //it's not a valid time, display blank
            $default_value_1 = '';
            $default_value_2 = '';
            $default_value_3 = '';
            $selected_am = '';
...
EOT;
    ...
    $element_markup = <<<EOT
		...
		<span class="description">{$element->title} {$span_required}</span>
		<span>
			<input id="element_{$element->id}_1" name="element_{$element->id}_1" {$attr_readonly} class="element text " size="2" type="text" maxlength="2" value="{$default_value_1}" /> : 
...
function mf_display_money($element)
{
    ...
    if ($element->constraint != 'yen')
    {
		...
		<span class="main_currency">
			<input id="element_{$element->id}_1" name="element_{$element->id}_1" {$attr_readonly} class="element text currency" size="10" value="{$default_value_1}" type="text" />		
...
EOT;
    ...
    { //for yen, only display one textfield
...
function mf_display_number($element)
{
    ...
    $element_markup = <<<EOT
		...
		<label class="description" for="element_{$element->id}">{$element->title} {$span_required}</label>
		<div>
			<input id="element_{$element->id}" name="element_{$element->id}" class="element text {$element->size}" {$attr_placeholder} {$attr_readonly} type="text" {$maxlength} {$quantity_link_data_tag} value="{$element->default_value}" {$input_handler} /> 
			{$range_limit_markup}
		</div>{$guidelines} {$error_message}
		</li>
EOT;
...
function mf_display_media($element)
{
    ...
    if ($element->media_type == 'image' && !empty($element->media_image_src))
    {
        ...
        {
            case 'l':
                $alignment_class = 'media_image_left';
            break;
            case 'c':
                $alignment_class = 'media_image_center';
            break;
            case 'r':
                $alignment_class = 'media_image_right';
            break;
        }
...
EOT;
    ...
    elseif ($element->media_type == 'video' && !empty($element->media_video_src))
    {
        ...
        if (in_array($media_info_host, array(
            'www.youtube.com',
            'youtube.com',
            'youtu.be'
        )))
...
EOT;
    ...
    elseif ($element->media_type == 'pdf' && !empty($element->media_pdf_src) && !$element->is_design_mode)
    {
        ...
        {
            case 'small':
                $size_class = 'media_pdf_small';
            break;
            case 'medium':
                $size_class = 'media_pdf_medium';
            break;
            case 'large':
                $size_class = 'media_pdf_large';
            break;
        }
...
function mf_get_country_list()
{
    ...
    $country[92]['label'] = "Jordan";
    $country[93]['label'] = "Kazakhstan";
    $country[94]['label'] = "Kenya";
    $country[95]['label'] = "Kiribati";
    $country[96]['label'] = "North Korea";
    $country[97]['label'] = "South Korea";
    ...
    $country[136]['label'] = "Oman";
    $country[137]['label'] = "Pakistan";
    $country[138]['label'] = "Palau";
    $country[139]['label'] = "Palestine";
    $country[140]['label'] = "Panama";
    $country[141]['label'] = "Papua New Guinea";
    $country[142]['label'] = "Paraguay";
    $country[143]['label'] = "Peru";
    $country[144]['label'] = "Philippines";
    $country[145]['label'] = "Poland";
    $country[146]['label'] = "Portugal";
    $country[147]['label'] = "Puerto Rico";
    $country[148]['label'] = "Qatar";
    $country[149]['label'] = "Romania";
    $country[150]['label'] = "Russia";
    $country[151]['label'] = "Rwanda";
    $country[152]['label'] = "Saint Kitts and Nevis";
    $country[153]['label'] = "Saint Lucia";
    $country[154]['label'] = "Saint Vincent and the Grenadines";
    $country[155]['label'] = "Samoa";
    $country[156]['label'] = "San Marino";
    $country[157]['label'] = "Sao Tome and Principe";
    $country[158]['label'] = "Saudi Arabia";
    $country[159]['label'] = "Senegal";
    $country[160]['label'] = "Serbia and Montenegro";
    $country[161]['label'] = "Seychelles";
    $country[162]['label'] = "Sierra Leone";
    $country[163]['label'] = "Singapore";
    $country[164]['label'] = "Slovakia";
    $country[165]['label'] = "Slovenia";
    $country[166]['label'] = "Solomon Islands";
    $country[167]['label'] = "Somalia";
    $country[168]['label'] = "South Africa";
    $country[169]['label'] = "Spain";
    $country[170]['label'] = "Sri Lanka";
    $country[171]['label'] = "Sudan";
    $country[172]['label'] = "Suriname";
    $country[173]['label'] = "Swaziland";
    $country[174]['label'] = "Sweden";
    $country[175]['label'] = "Switzerland";
    $country[176]['label'] = "Syria";
    $country[177]['label'] = "Taiwan";
    $country[178]['label'] = "Tajikistan";
    $country[179]['label'] = "Tanzania";
    $country[180]['label'] = "Thailand";
    $country[181]['label'] = "Togo";
    $country[182]['label'] = "Tonga";
    $country[183]['label'] = "Trinidad and Tobago";
    $country[184]['label'] = "Tunisia";
    $country[185]['label'] = "Turkey";
    $country[186]['label'] = "Turkmenistan";
    $country[187]['label'] = "Tuvalu";
    $country[188]['label'] = "Uganda";
    $country[189]['label'] = "Ukraine";
    $country[190]['label'] = "United Arab Emirates";
    $country[191]['label'] = "Uruguay";
    $country[192]['label'] = "Uzbekistan";
    $country[193]['label'] = "Vanuatu";
    $country[194]['label'] = "Vatican City";
    $country[195]['label'] = "Venezuela";
    $country[196]['label'] = "Vietnam";
    $country[197]['label'] = "Yemen";
    $country[198]['label'] = "Zambia";
    $country[199]['label'] = "Zimbabwe";
    ...
    $country[92]['value'] = "Jordan";
    $country[93]['value'] = "Kazakhstan";
    $country[94]['value'] = "Kenya";
    $country[95]['value'] = "Kiribati";
    $country[96]['value'] = "North Korea";
    $country[97]['value'] = "South Korea";
    ...
    $country[136]['value'] = "Oman";
    $country[137]['value'] = "Pakistan";
    $country[138]['value'] = "Palau";
    $country[139]['value'] = "Palestine";
    $country[140]['value'] = "Panama";
    $country[141]['value'] = "Papua New Guinea";
    $country[142]['value'] = "Paraguay";
    $country[143]['value'] = "Peru";
    $country[144]['value'] = "Philippines";
    $country[145]['value'] = "Poland";
    $country[146]['value'] = "Portugal";
    $country[147]['value'] = "Puerto Rico";
    $country[148]['value'] = "Qatar";
    $country[149]['value'] = "Romania";
    $country[150]['value'] = "Russia";
    $country[151]['value'] = "Rwanda";
    $country[152]['value'] = "Saint Kitts and Nevis";
    $country[153]['value'] = "Saint Lucia";
    $country[154]['value'] = "Saint Vincent and the Grenadines";
    $country[155]['value'] = "Samoa";
    $country[156]['value'] = "San Marino";
    $country[157]['value'] = "Sao Tome and Principe";
    $country[158]['value'] = "Saudi Arabia";
    $country[159]['value'] = "Senegal";
    $country[160]['value'] = "Serbia and Montenegro";
    $country[161]['value'] = "Seychelles";
    $country[162]['value'] = "Sierra Leone";
    $country[163]['value'] = "Singapore";
    $country[164]['value'] = "Slovakia";
    $country[165]['value'] = "Slovenia";
    $country[166]['value'] = "Solomon Islands";
    $country[167]['value'] = "Somalia";
    $country[168]['value'] = "South Africa";
    $country[169]['value'] = "Spain";
    $country[170]['value'] = "Sri Lanka";
    $country[171]['value'] = "Sudan";
    $country[172]['value'] = "Suriname";
    $country[173]['value'] = "Swaziland";
    $country[174]['value'] = "Sweden";
    $country[175]['value'] = "Switzerland";
    $country[176]['value'] = "Syria";
    $country[177]['value'] = "Taiwan";
    $country[178]['value'] = "Tajikistan";
    $country[179]['value'] = "Tanzania";
    $country[180]['value'] = "Thailand";
    $country[181]['value'] = "Togo";
    $country[182]['value'] = "Tonga";
    $country[183]['value'] = "Trinidad and Tobago";
    $country[184]['value'] = "Tunisia";
    $country[185]['value'] = "Turkey";
    $country[186]['value'] = "Turkmenistan";
    $country[187]['value'] = "Tuvalu";
    $country[188]['value'] = "Uganda";
    $country[189]['value'] = "Ukraine";
    $country[190]['value'] = "United Arab Emirates";
    $country[191]['value'] = "Uruguay";
    $country[192]['value'] = "Uzbekistan";
    $country[193]['value'] = "Vanuatu";
    $country[194]['value'] = "Vatican City";
    $country[195]['value'] = "Venezuela";
    $country[196]['value'] = "Vietnam";
    $country[197]['value'] = "Yemen";
    $country[198]['value'] = "Zambia";
    $country[199]['value'] = "Zimbabwe";
    return $country;
}
//Address
    ...
    $element_markup = <<<EOT
		...
		<div>
			<span id="li_{$element->id}_span_1">
				<input id="element_{$element->id}_1" name="element_{$element->id}_1" class="element text large" {$attr_readonly} value="{$default_value_1}" type="text" />
...
function mf_display_captcha($element)
{
    ...
    { //if this is internal captcha type
        ...
        $captcha_html = <<<EOT
<img id="captcha_image" src="{$machform_path}captcha.php?t={$timestamp}&mfsid={$mfsid}" width="200" height="60" alt="Please refresh your browser to see this image." /><br />
...
function mf_display_matrix($element)
{
    ...
    foreach ($element->options as $option)
    {
        ...
        $first_row_td .= "<td><fieldset><legend style=\"display: none\">{$element->title}</legend><label style=\"display: none\" for=\"element_{$element->id}_{$option->id}\">{$option_titles[$i]}</label><input id=\"element_{$element->id}_{$option->id}\" name=\"element_{$element->id}{$option_id_var}\" type=\"{$input_type}\" value=\"{$option->id}\" {$checked_markup} /></fieldset></td>\n";
        $i++;
    }
...
function mf_display_form($dbh, $form_id, $form_params = array())
{
    ...
    if (!empty($form->logic_page_enable) && !empty($page_number))
    {
        ...
        if ($page_number == 1)
        { //if there is no current history, initialize with page 1
    ...
    if ($form->page_total == 1)
    {
        ...
        if (!empty($_GET['mf_edit']) && !empty($form->entry_edit_enable))
        {
            ...
            if (!empty($edit_key_entry_id))
            {
                ...
                {
                    if ($form->entry_edit_auto_disable_unit == 'r')
                    { //expiry based on x revisions
                        $query = "SELECT 
                    ...
                    else if ($form->entry_edit_auto_disable_unit == 'h')
                    { //expiry based on x hours
                    ...
                    else if ($form->entry_edit_auto_disable_unit == 'd')
                    { //expiry based on x days
                        $form->entry_edit_auto_disable_period = (int)$form->entry_edit_auto_disable_period;
                        $query = "SELECT
    ...
    while ($row = mf_do_fetch_result($sth))
    {
        ...
        if (!empty($row['element_guidelines']) && ($row['element_is_private'] == 0) && !in_array($row['element_type'], array(
            'section',
            'media',
            'matrix',
            'signature'
        ...
        if ($row['element_type'] == 'file')
        {
            ...
            if ((!empty($form->review) && !empty($_SESSION['review_id']) && !empty($populated_file_values)) || ($form->page_total > 1 && !empty($populated_file_values)) || (!empty($_SESSION['mf_form_edit_key'][$form_id]) && !empty($populated_file_values)))
            {
                //populate the default value for uploaded files, when validation error occured
            ...
            if (!empty($row['element_date_enable_selection_limit']) && !empty($row['element_date_selection_max']))
            {
                $sub_query = "select 
								   ...
								   where 
										 A.total_selection >= ?";
                $params = array(
                    $row['element_date_selection_max']
                );
        ...
        if ($form->page_total == 1 || ($form->page_total == $page_number))
        {
            if ($_SESSION['captcha_passed'][$form_id] !== true)
            {
                ...
                if ($form->captcha_type == 'n')
                {
                    ...
                    {
                        case 'bulgarian':
                            $recaptcha2_language = 'bg';
                        break;
                        case 'chinese':
                            $recaptcha2_language = 'zh-TW';
                        break;
                        case 'chinese_simplified':
                            $recaptcha2_language = 'zh-CN';
                        break;
                        case 'danish':
                            $recaptcha2_language = 'da';
                        break;
                        case 'dutch':
                            $recaptcha2_language = 'nl';
                        break;
                        case 'estonian':
                            $recaptcha2_language = 'en';
                        break;
                        case 'finnish':
                            $recaptcha2_language = 'fi';
                        break;
                        case 'french':
                            $recaptcha2_language = 'fr';
                        break;
                        case 'german':
                            $recaptcha2_language = 'de';
                        break;
                        case 'greek':
                            $recaptcha2_language = 'el';
                        break;
                        case 'hungarian':
                            $recaptcha2_language = 'hu';
                        break;
                        case 'indonesia':
                            $recaptcha2_language = 'id';
                        break;
                        case 'italian':
                            $recaptcha2_language = 'it';
                        break;
                        case 'japanese':
                            $recaptcha2_language = 'ja';
                        break;
                        case 'norwegian':
                            $recaptcha2_language = 'no';
                        break;
                        case 'polish':
                            $recaptcha2_language = 'pl';
                        break;
                        case 'portuguese':
                            $recaptcha2_language = 'pt';
                        break;
                        case 'romanian':
                            $recaptcha2_language = 'ro';
                        break;
                        case 'russian':
                            $recaptcha2_language = 'ru';
                        break;
                        case 'slovak':
                            $recaptcha2_language = 'sk';
                        break;
                        case 'spanish':
                            $recaptcha2_language = 'es';
                        break;
                        case 'swedish':
                            $recaptcha2_language = 'sv';
                        break;
                        default:
                            $recaptcha2_language = 'en';
                        break;
    ...
    {
        if (($element_data->is_private == 1) && empty($edit_id))
...
EOT;
    ...
    if ((empty($form->active) || $form_has_maximum_entries) && $is_edit_entry === false)
...
EOT;
    ...
    if ($has_calendar)
    {
        ...
        {
            case 'dutch':
                $datepick_lang = 'js/datepick5/jquery.datepick-nl.js';
            break;
            case 'french':
                $datepick_lang = 'js/datepick5/jquery.datepick-fr.js';
            break;
            case 'german':
                $datepick_lang = 'js/datepick5/jquery.datepick-de.js';
            break;
            case 'italian':
                $datepick_lang = 'js/datepick5/jquery.datepick-it.js';
            break;
            case 'portuguese':
                $datepick_lang = 'js/datepick5/jquery.datepick-pt.js';
            break;
            case 'spanish':
                $datepick_lang = 'js/datepick5/jquery.datepick-es.js';
            break;
            case 'swedish':
                $datepick_lang = 'js/datepick5/jquery.datepick-sv.js';
            break;
            case 'japanese':
                $datepick_lang = 'js/datepick5/jquery.datepick-ja.js';
            break;
            case 'estonian':
                $datepick_lang = 'js/datepick5/jquery.datepick-et.js';
            break;
            case 'russian':
                $datepick_lang = 'js/datepick5/jquery.datepick-ru.js';
            break;
            case 'hungarian':
                $datepick_lang = 'js/datepick5/jquery.datepick-hu.js';
            break;
            case 'chinese':
                $datepick_lang = 'js/datepick5/jquery.datepick-zh-TW.js';
            break;
            case 'chinese_simplified':
                $datepick_lang = 'js/datepick5/jquery.datepick-zh-CN.js';
            break;
            case 'bulgarian':
                $datepick_lang = 'js/datepick5/jquery.datepick.js-bg';
            break;
            case 'danish':
                $datepick_lang = 'js/datepick5/jquery.datepick-da.js';
            break;
            case 'finnish':
                $datepick_lang = 'js/datepick5/jquery.datepick-fi.js';
            break;
            case 'polish':
                $datepick_lang = 'js/datepick5/jquery.datepick-pl.js';
            break;
            case 'greek':
                $datepick_lang = 'js/datepick5/jquery.datepick-el.js';
            break;
            case 'norwegian':
                $datepick_lang = 'js/datepick5/jquery.datepick-no.js';
            break;
            case 'romanian':
                $datepick_lang = 'js/datepick5/jquery.datepick-ro.js';
            break;
            case 'slovak':
                $datepick_lang = 'js/datepick5/jquery.datepick-sk.js';
            break;
            case 'indonesia':
                $datepick_lang = 'js/datepick5/jquery.datepick-id.js';
            break;
            default:
                $datepick_lang = ''; //the default english language
        }
...
function mf_display_form_review($dbh, $form_id, $record_id, $from_page_num, $form_params = array())
{
    ...
    foreach ($entry_details as $data)
    {
        ...
        if ($data['element_type'] == 'section' || $data['element_type'] == 'media')
        {
            ...
            if ((!empty($hidden_elements) && !empty($hidden_elements[$data['element_id']])) || (!empty($hidden_elements) && !empty($hidden_elements[$data['matrix_main_element_id']])))
            {
                continue;
            }
...
function mf_display_form_payment($dbh, $form_id, $record_id, $form_params = array())
{
    ...
    if (in_array($payment_merchant_type, array(
        'braintree'
    )))
    {
...
EOT;
        ...
        if (!empty($payment_enable_setupfee) && !empty($payment_setupfee_amount) && in_array($payment_merchant_type, array(
            'stripe',
            'paypal_rest'
        )))
        {
...
EOT;
    ...
    $accepted_card_types = array(
        ...
        'discover',
        'diners'
    ); //the default accepted credit card types
    ...
    $credit_card_markup = <<<EOT
				<li id="li_credit_card" class="credit_card">
					<fieldset>
						<legend style="display: none">{$mf_lang['payment_card_title']}</legend>
...
EOT;
        ...
        if (!empty($payment_stripe_enable_payment_request_button))
        {
            ...
            $payment_intent = \Stripe\PaymentIntent::create(['amount' => $mf_stripe_total_payment_amount, 'currency' => $mf_stripe_payment_currency, ]);
